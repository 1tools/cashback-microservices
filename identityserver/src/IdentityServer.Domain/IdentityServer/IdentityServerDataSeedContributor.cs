...
public class IdentityServerDataSeedContributor : IDataSeedContributor, ITransientDependency
{
    ...
    private async Task CreateApiScopesAsync()
    {
        ...
        await CreateApiScopeAsync("CashbackService");
    }
    ...
    private async Task CreateApiResourcesAsync()
    {
        ...
        await CreateApiResourceAsync("CashbackService", commonApiUserClaims);
    }
    ...
    private async Task CreateClientsAsync()
    {
        ...
        // Create public website client
        await CreateClientAsync(
                name: "PublicWebsiteClient",
                scopes: commonScopes.Union(new[] { "PublicGateway", "ProductService", "PaymentService" }),
                grantTypes: new[] { "password", "client_credentials", "authorization_code" },
                secret: "1q2w3e*".Sha256(),
                requireClientSecret: false,
                redirectUri: "https://localhost:5000",
                postLogoutRedirectUri: "https://localhost:5000",
                corsOrigins: new[] { "https://localhost:5000" }
            );

        // Create Admin website client
        await CreateClientAsync(
                name: "AdminWebsiteClient",
                scopes: commonScopes.Union(new[] { "PublicGateway", "ProductService", "PaymentService", "CashbackService" }),
                grantTypes: new[] { "password", "client_credentials", "authorization_code" },
                secret: "1q2w3e*".Sha256(),
                requireClientSecret: false,
                redirectUri: "https://localhost:5001",
                postLogoutRedirectUri: "https://localhost:5001",
                corsOrigins: new[] { "https://localhost:5001" }
            );

        // Configuation web (internal only)
        await CreateClientAsync(
                name: "ConfiguationWebsiteClient",
                scopes: commonScopes.Union(new[] { "InternalGateway", "ProductService", "PaymentService", "CashbackService" }),
                grantTypes: new[] { "password", "client_credentials", "authorization_code" },
                secret: "1q2w3e*".Sha256(),
                requireClientSecret: false,
                redirectUri: "https://localhost:5002",
                postLogoutRedirectUri: "https://localhost:5002",
                corsOrigins: new[] { "https://localhost:5002" }
            );
