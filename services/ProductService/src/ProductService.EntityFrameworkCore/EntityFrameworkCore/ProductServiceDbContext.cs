    ...
    ITenantManagementDbContext
{
    ...
    {
        // Config product table
        builder.Entity<Product>().HasOne<Category>(t => t.Category).WithMany(c => c.Products).HasForeignKey(p => p.CategoryId);
        builder.Entity<Product>().HasOne<Manufacturer>(t => t.Manufacturer).WithMany(c => c.Products).HasForeignKey(p => p.ManufacturerId);
    }
}
